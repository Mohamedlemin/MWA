...
export class PmplDataService {
  ...
  private BASE_URL = environment.BASE_URL+"/"+environment.pmpls;
  ...
  getAll(offset: number, count: number): Observable<pmpl[]> {
    return this._http.get<pmpl[]>(this.BASE_URL + environment.offset + offset + environment.count + count);
  }
  ...
  getTeam(pmplId: string, teamID: string): Observable<Team> {
    return this._http.get<Team>(this.BASE_URL+'/' + pmplId + "/"+environment.teams+"/" + teamID)
  }
